...
#     """
#     def __init__(self, output_dim=768, pretrained=True, model_variant='L2'):
#         super().__init__()
...
#         elif model_variant == 'L2':
#             self.mamba_vision = mamba_vision_L2(pretrained=pretrained, model_path="/tmp/mambavision_large2_1k.pth.tar")
# #         # ... 你可以添加更多模型的elif分支 ...
#         # ... 你可以添加更多模型的elif分支 ...
#         else:
...
class MambaVision(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for level in self.levels:
            x = level(x)
        #print(f"After levels(x) shape: {x.shape}")
        # exit(0)
        x = self.norm(x)
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
    ...
    def forward_features_mamba_fusion(self, x):
        x = self.patch_embed(x)
        #print(f"patch_embed(x) shape: {x.shape}")
        for level in self.levels:
            x = level(x)
...
class MambaVisionEncoder(nn.Module):
    ...
    - 使用 forward_features() 提取特征。
    - 添加一个投影层，将特征维度映射到期望的输出维度 (512)。
    但是使用CRM以及其他以特征图为输入的模块时，不能加上线性投影层
    """
    def __init__(self, output_dim=768, pretrained=True, model_variant='L2'):
        super().__init__()
        ...
        if model_variant == 'S':
            ...
            self.mamba_vision = mamba_vision_T(pretrained=pretrained, model_path="/tmp/mamba_vision_T.pth.tar")
        elif model_variant == 'L3-512-21K':
            self.mamba_vision = mamba_vision_L3_512_21k(pretrained=pretrained, model_path="/tmp/mamba_vision_L3_512_21K.pth.tar")
        elif model_variant == 'L2':
            self.mamba_vision = mamba_vision_L2(pretrained=pretrained, model_path="/data/sb/aaa_final_isic/MambaVision-L2-512-21K/mamba_vision_L2_512_21k.pth.tar")
        # ... 你可以添加更多模型的elif分支 ...
        else:
            raise ValueError(f"Unsupported MambaVision variant: {model_variant}")
    ...
    def forward(self, x):
        # 1. 使用 mamba_vision 提取特征
        # print(f"Input shape: {x.shape}")#Input shape: torch.Size([16, 3, 224, 224])
        # exit()
        # forward_features 的输出 shape: (batch_size, feature_dim)

        #features = self.mamba_vision.forward_features(x)
        projected_features = self.mamba_vision.forward_features_mamba_fusion(x)#patch_embed(x) shape: torch.Size([16, 80, 56, 56])  After levels(x) shape: torch.Size([16, 640, 7, 7])
        # print(f"projected_features shape: {projected_features.shape}")# projected_features shape: torch.Size([16, 640, 7, 7])
        # exit()
        batch_size = projected_features.shape[0]  
        projected_features = projected_features.reshape(batch_size, 1568,-1)  # 调整维度顺序为 (batch_size, height, width, feature_dim)
        # print(f"Extracted features shape: {projected_features.shape}")
        # exit(0)
...
#
#----------------------------------
def create_mamba_vision_encoder(
    output_dim=512,
    pretrained=True,
    model_variant='L2',
    model_paths=None
    ...
    """
    创建并返回 MambaVision 图像编码器
    
    参数：
        output_dim: 输出特征维度 (默认512)
        pretrained: 是否加载预训练权重 (默认True)
        model_variant: 模型变体 (默认为'L2')
        model_paths: 自定义模型路径字典 (可选)
    
    支持变体: 
        'T', 'T2', 'S', 'B', 'B_21k', 'L', 'L_21k', 'L2', 
        'L2_512_21k', 'L3_256_21k', 'L3_512_21k'
    """
    # 默认模型路径映射
    default_paths = {
        'T': "/tmp/mamba_vision_T.pth.tar",
        'T2': "/tmp/mamba_vision_T2.pth.tar",
        'S': "/tmp/mamba_vision_S.pth.tar",
        'B': "/tmp/mamba_vision_B.pth.tar",
        'B_21k': "/tmp/mamba_vision_B_21k.pth.tar",
        'L': "/tmp/mamba_vision_L.pth.tar",
        'L_21k': "/tmp/mamba_vision_L_21k.pth.tar",
        'L2': "/tmp/mamba_vision_L2.pth.tar",
        'L2_512_21k': "/tmp/mamba_vision_L2_512_21k.pth.tar",
        'L3_256_21k': "/tmp/mamba_vision_L3_256_21k.pth.tar",
        'L3_512_21k': "/tmp/mamba_vision_L3_512_21k.pth.tar",
    }
    
    # 使用自定义路径或默认路径
    model_path = model_paths.get(model_variant, default_paths.get(model_variant)) if model_paths else default_paths.get(model_variant)
...
if __name__ == "__main__":
    # 创建编码器（使用默认参数）
    encoder = create_mamba_vision_encoder()
    
    # 测试示例输入
    input_tensor = torch.randn(2, 3, 224, 224) # 批大小2, 3通道, 224x224图像
    features = encoder(input_tensor)
    print(f"Output features shape: {features.shape}")  # 应输出 torch.Size([2, 512])